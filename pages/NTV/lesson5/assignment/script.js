...
function revealCell(row, col) {
  const revealedCell = gameBoard[row][col];
  if (revealedCell.classList.contains('revealed')) {
    return
  } else {
    ...
    const hasBomb = bombPositions.includes(cellIndex)
    if (hasBomb) {
      revealedCell.classList.add('bomb')
      return endGame(false)
    }

    const bombsAround = countBombsAround(row, col)
    if (bombsAround > 0) {
      revealedCell.textContent = bombsAround
    } else {
